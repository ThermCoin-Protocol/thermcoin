...
RUN apt-get update && \
    ...
    cp src/ThermCoind /usr/local/bin/ThermCoind && \
    cp src/walletd /usr/local/bin/walletd && \
    cp src/zedwallet /usr/local/bin/zedwallet && \
    cp src/miner /usr/local/bin/miner && \
    strip /usr/local/bin/ThermCoind && \
...
RUN useradd -r -s /usr/sbin/nologin -m -d /var/lib/turtlecoind turtlecoind && \
    ...
    echo "s6-setuidgid turtlecoind /usr/local/bin/ThermCoind" >> /etc/services.d/turtlecoind/run && \
